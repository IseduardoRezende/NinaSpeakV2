...
{    
    ...
    {
        Task<IEnumerable<TEntity>> GetAsync(Func<TEntity, bool> filters, bool ignoreGlobalFilter = false, params string[] includes);

        Task<TEntity?> GetByIdsAsync(long[] ids, params string[] includes);
    
        Task<TEntity?> GetByIdAsync(long id, params string[] includes);   

        Task<TEntity?> GetByAsync(Func<TEntity, bool> filters, bool ignoreGlobalFilter = false, params string[] includes);
    }
}
